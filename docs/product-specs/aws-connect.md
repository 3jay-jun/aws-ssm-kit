...
1. 사용자가 프로필 이름, Region, Account ID, IAM User ID, Access Key, Secret Key를 입력한다.
...
3. 만료, 불일치, 파싱 오류 또는 만료 임박이면 MFA 6자리 코드를 요청한다.
4. STS `GetSessionToken`으로 새 임시 자격증명을 발급한다.
...
5. 사용자의 명시적 동작이 있을 때만 개별 값을 일시적으로 표시하거나 복사한다.
...
8. `ssm:SendCommand` 권한이 없으면 사용자가 명시적으로 선택해 `ssm:StartSession` 기반 외부
   터미널에서 같은 고정 명령을 실행할 수 있다. 결과는 앱이 회수하거나 저장하지 않는다.
9. 조회한 JSON의 최상위 키는 명시적 확인 후 `PutSecretValue`로 새 버전을 저장할 수 있다.
...
- FR-SECRET-007: JSON 최상위 키 변경은 명시적 확인 후 AWS 새 버전으로 저장하며 원문을
  SQLite나 로그에 기록하지 않아야 한다.
