...
5. 사용자의 명시적 동작이 있을 때만 개별 값을 30초 동안 일시적으로 표시한다. GUI는 값 복사와
   AWS 키-값 수정 버튼을 제공하지 않는다.
...
9. `ListSecrets` 권한이 있으면 상단 검색을 선택 상자로 제공하고, 없으면 이름/ARN 직접 입력으로
   전환한다.
10. 조회에 성공한 Secret 이름/ARN은 프로필별 SQLite 저장 목록에 자동 등록한다. 왼쪽 패널에서
    저장 항목을 등록·수정·삭제하며 Secret 원문은 저장하지 않는다. 오른쪽은 선택한 값 확인만
    제공한다. 중계 EC2 목록은 프로필 선택 시 자동으로 조회한다.

### 7.6 S3 조회 및 업로드

1. Bucket 목록 권한이 있으면 선택 상자를, 없으면 저장 위치 또는 직접 Bucket 입력을 사용한다.
2. 접근 가능한 객체 목록을 조회한다.
3. 로컬 파일 선택 창에서 업로드할 파일을 선택한다.
4. 대상 S3 URI와 덮어쓰기 여부를 사전에 표시한다.
5. 동일 키가 있으면 사용자 확인 후 덮어쓴다.
6. 업로드 진행률과 완료 결과를 표시한다.
7. 대용량 파일은 multipart upload를 사용한다.
8. 선택한 단일 객체는 명시적 확인 후 삭제할 수 있으며 prefix 재귀 삭제는 제공하지 않는다.
9. 선택한 파일은 사용자가 고른 로컬 경로에 원자적으로 다운로드한 뒤 기본 연결 프로그램으로
   열 수 있다. 업로드 완료와 삭제 완료 후 현재 목록을 즉시 다시 조회한다.
...
- FR-SECRET-004: Secret 원문을 로그나 SQLite에 저장하지 않아야 한다.
...
- FR-SECRET-007: 조회 성공 시 이름/ARN만 프로필별 SQLite 목록에 자동 등록하고 저장 항목의
  등록·수정·삭제를 제공해야 한다. Secret 원문과 필드 값은 저장하지 않아야 한다.
...
- FR-S3-003: 단일 및 다중 파일 업로드를 지원해야 한다.
- FR-S3-004: 기존 객체 덮어쓰기 전에 확인해야 한다.
...
- FR-S3-006: `ListAllMyBuckets` 성공 시 Bucket 선택 상자를 제공하고 권한 거부 시 직접 입력을
  유지해야 한다.
- FR-S3-007: 선택한 객체를 사용자 지정 경로로 다운로드한 뒤 기본 연결 프로그램으로 열 수
  있어야 하며 실패한 부분 파일은 남기지 않아야 한다.
...
- 인증정보와 Secret 원문이 DB 및 로그에서 발견되지 않는다.
