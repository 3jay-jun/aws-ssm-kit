...
class AuthenticatedOperationCoordinator:
    ...
    def start_long[T](
        ...
        self._sweep_expired()
        attempted = self._invoke(initial.operation_id, lambda: action(initial), initial)
        if (
        ...
        with execution_scope(initial.correlation_id, initial.operation_id):
        ...
        if refresh.state is OperationState.SUCCEEDED:
            return self._invoke(refresh.operation_id, lambda: action(rebound), rebound)
        ...
        with self._lock:
            ...
            if not at_capacity:
                self._pending[refresh.operation_id] = _PendingAction(
                    refresh.challenge.expires_at, lambda: action(rebound), rebound
                )
        if at_capacity:
    ...
    def resume[T](self, operation_id: str, mfa_code: str | None) -> OperationResult[T]:
        ...
        with execution_scope(pending.context.correlation_id, operation_id):
        ...
        return self._invoke(operation_id, pending.action, pending.context)  # type: ignore[arg-type]
    ...
    @staticmethod
    def _invoke[T](
        operation_id: str, action: Callable[[], T], context: OperationContext | None = None
    ) -> OperationResult[T]:
        return execute_operation(
            lambda _context: action(), context or OperationContext(operation_id=operation_id)
        )
