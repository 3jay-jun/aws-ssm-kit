...
1. EC2 inventory와 SSM 관리 노드를 Instance ID로 합치고 EC2 전원/SSM 준비 상태를 별도 배지로 표시한다.
2. 전체·실행 중·중지됨, 이름/ID/IP 검색, 즐겨찾기만 필터를 조합한다. 즐겨찾기는 프로필·Region별로 저장한다.
3. EC2 조회 권한이 없으면 기존 SSM Online 정보로 축소한다.
4. 준비된 대상의 셸은 공통 인증/MFA 검증 후 외부 Windows Terminal에서 연다.
5. 중지된 대상의 시작은 유지하고, GUI의 SSM 불가 대상은 안내와 비활성 버튼을 표시한다.
   기존 CLI 재부팅 계약은 유지한다. Stop/Terminate/대량 전원 작업은 추가하지 않는다.
6. 최근 접속은 보관 활동 로그의 프로필·Region별 접속 성공 이벤트만 재사용한다.
   구형 로그에 명확한 식별 정보가 없으면 이력 없음으로 표시한다.
7. 더보기에서 ID/IP 복사와 조회 전용 태그 팝업을 제공하며, 하단은 선택 인스턴스와 마지막 성공 요약이다.
...
3. 우측 상단 folder+ 버튼 또는 드래그 앤 드롭으로 로컬 파일과 폴더를 누적 선택하고 업로드 전 간단한 목록으로 확인한다.
...
- FR-S3-008: S3 Bucket 헤더에 Bucket/breadcrumb를 통합하고 고정 Prefix 입력은 검색/경로 이동 모달로 이동한다.
- FR-S3-009: 현재 Prefix 하위 파일명 부분 검색, 최근 경로, 단일 파일 이름 변경과 S3 URI 복사를 제공한다.
  이름 변경은 기존 key를 덮어쓰지 않으며 5 GiB 이하 파일만 지원한다. 복사 성공/삭제 실패를 구분한다.
- FR-S3-010: 업로드 큐에서 성공/실패/대기, 파일별 진행률/시작 시각, 실패 재시도, 완료 숨김/실패 필터와 로컬 목록 비우기를 제공한다.
  폴더 확장/충돌 판단은 기존 S3Service만 담당하며 재시도는 원 대상과 인증/충돌 흐름을 보존한다.
...
- 업로드 큐는 표로 표시하며 파일명, 썸네일/아이콘, 크기, 로컬 경로, 수정 시간, 상태, 진행률과 행 작업을 제공한다.
- 다운로드 대상에 기존 파일이 있으면 덮어쓰기·무시·취소를 선택하며, 미승인 덮어쓰기를 금지한다. 완료 안내에 다운로드/무시 개수를 표시한다.

### Secrets 조회 화면 보완 (2026-09-17)

직접/EC2 경유 조회를 조건부 UI로 구분한다. EC2 경유만 중계 연결 확인과 명시적 동의를 제공하며,
실패 상태에서는 조회를 차단한다. 저장 항목은 로컬 수동 생성/삭제와 조회 컨텍스트를 유지한다.
키-값/JSON 탭은 기본 마스킹, 명시적 30초 표시, 전체 숨기기 및 정책 준수 복사를 제공한다.
저장 Secret별 성공 조회 시각을 별도 보존하고 수동 수정 시각과 혼동하지 않는다.
AWS Secret 생성/수정/삭제 또는 DB 접속 기능은 추가하지 않는다.

## 구조화 실행 로그 (2026-09-18)

운영 이력은 SQLite execution_logs에 저장하며 성공/경고/실패/취소와 실행 단계를 독립적으로 관리한다. 인증·MFA·EC2·SSM·RDS·Secrets·S3·설정의 의미 있는 실행 경계를 기록하고 UI 선택·토스트 등은 제외한다. 파일 진단 로그는 유지한다. 보존은 30일/10,000건이며 과거 파일에서 성공을 추론하거나 이관하지 않는다. 대시보드 최근 작업은 완료 이벤트 조회를 재사용한다. 상세 검색은 메시지·대상·operation/correlation ID·AWS Action·오류 코드를 포함한다.


### 대시보드 권한 관측 및 최근 작업 (2026-09-18)

4개 상태 카드와 최근 완료 작업 5건을 제공한다. Application DashboardService의 DTO로
관측 결과를 전달한다. 최근 작업은 ExecutionLogService를 재사용하며 별도 저장하지 않는다.
권한 새로 확인은 EC2/SSM 인스턴스 정보, Secrets 목록, S3 버킷 목록 및
S3 탭에서 실제 조회한 마지막 버킷/Prefix의 파일 목록을 조회한다.
세션 시작·Secret 원문 조회·생성/수정/삭제·업로드를 검사용으로 실행하지 않는다.
미관측/네트워크/리소스/Agent 오류는 확인 필요이며, 정확한 API의 명시적 권한 거부만 불가이다.
저장소 보존 기간 내 동일 프로필/리전의 작업 이력은 관측 대상 범위로만 참고하며 대상별 상반된 결과는
확인 필요이다. 프로필/리전 없는 구형 성공 기록은 임의로 다른 프로필에 귀속하지 않는다.
S3 삭제는 DeleteObject의 성공/권한 오류만 참고한다. S3 화면에서 발생한 SSM 오류나
버킷 목록 권한을 삭제 권한으로 대체하지 않는다. 성공 기록 부재는 권한 거부가 아니다.
일반 업로드/덮어쓰기, multipart 완료, 이름 변경(CopyObject+DeleteObject)은 실제
성공한 객체별로 프로필/리전/버킷과 함께 저장한다. 성공한 CopyObject 및
CompleteMultipartUpload는 PutObject 사용 근거이며, 해당 API의 실패를 PutObject
거부로 단정하지 않는다. 마스킹/절단된 경로는 재조회하지 않는다.
Secret 생성/수정/삭제 UI, S3 일괄/재귀 삭제, EC2 Stop/Terminate는 추가하지 않는다.

대시보드로 돌아올 때 최근 작업과 권한 관측을 함께 갱신한다. 권한 행에는
가능/불가/확인 필요를 텍스트로 병기하며, 확인 필요는 권한 거부를 의미하지 않는다.

### EC2 시작 상태 및 외부 Terminal 종료 (2026-09-21)
- 설명은 `EC2 인스턴스를 조회하고 SSM을 통해 Windows Terminal에서 셸을 실행합니다.`이다.
- Terminal 작업은 검정 배경과 흰색 >_ 아이콘, 더보기는 S3/RDS와 동일한 투명
  아이콘 버튼이다. Region 입력은 최소 210px 및 내부 padding으로 기본 리전이 잘리지 않는다.
- EC2 목록은 pending/running/stopping/stopped를 포함한다. 시작한 행은 보존하며
  상태 필터 값은 변경하지 않는다. 검색/즐겨찾기는 계속 적용하고 완료 행의 임시
  표시는 사용자가 필터를 변경할 때 해제한다. 5초마다 최대 10분 동안 EC2와 SSM을
  확인하고 running+SSM Online에서 멈춘다. 시작 직후 stale stopped는 15초만 허용한다.
  프로필/리전 변경은 추적을 초기화하고 이전 범위의 성공 응답은 반영하지 않는다.
- 최근 접속은 SQLite ExecutionLogService의 보존된 SUCCESS 접속 이벤트를
  Profile+Region+Instance ID별로 조회하며 `YYYY-MM-DD HH:mm`으로 표시한다.
  StartSession 성공 및 외부 Plugin 실행 이후만 기록하며 시작 실패는 갱신하지 않는다.
  별도 테이블은 추가하지 않으며 기존 실행 로그 보존 정책(30일/최대 10,000행)을 따른다.
- 명시적 Windows console close와 exit 0은 정상 종료로 처리한다. 예고 없는 IPC
  단절, non-zero Plugin 종료는 실패로 유지한다. 정상 종료 기록은 INFO/SUCCESS이며
  정상 종료 후 AWS 정리 요청만 실패하면 WARNING과 비차단 안내로 구분한다.

### S3 목록/다운로드 표시 보정 (2026-09-21)
- Bucket 선택은 최소 380px, 내용 기반 폭 조정(최대 480px), 가로 padding 축소,
  전체 이름 tooltip을 제공한다.
- 객체 표의 독립 상태 컬럼에 다운로드 중 회전 아이콘/성공 초록 체크/실패 빨간 X를
  표시한다. 작업 더보기는 별도 컬럼이다. 프로필 변경 시 상태를 초기화한다.
- 객체 및 업로드 메뉴 항목은 동일 버튼 높이/padding을 사용하며 삭제는 색상만 빨강이다.
- 업로드 원본의 실제 로컬 경로와 filesystem st_mtime을 표시한다. 긴 로컬 경로는
  ellipsis 및 전체 tooltip으로 표시한다. S3 대상은 기존 plan/재시도 로직에서 유지한다.
- 업로드 빈 영역 클릭은 선택창을 열지 않는다. folder+ 버튼 및 drag/drop만 추가 진입점이다.
- 선택 파일은 다운로드 폴더 바로 아래 basename으로 저장한다. 폴더를 선택한 경우
  재귀 구조를 유지한다. 동일 basename 충돌은 기존 duplicate 오류로 전송 전에 거부하고,
  기존 파일은 overwrite/skip 선택 및 파일시스템 보호 정책을 계속 적용한다.

### Secrets 저장 목록 보정 (2026-09-21)
저장 개수 제목을 제거하고 이름/마지막 성공 조회 시각/더보기와 원본 Secret ID의 2줄로 표시한다.
현재 별도 별칭 모델은 없으므로 기존 Secret 이름을 유지한다. ID는 말줄임 및 전체 tooltip을 제공한다.
마지막 조회는 실제 값 조회 성공 시각이며 로컬 편집/목록 열기는 갱신하지 않는다.
더보기의 수정하기는 기존 더블클릭 수정 팝업을 공유하고 ID 복사/이름 복사/로컬 삭제를 유지한다.
