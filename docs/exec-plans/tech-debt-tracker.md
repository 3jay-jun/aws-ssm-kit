...
| QA-003 | Open | Qt full-suite isolation | 2026-09-17 전체 pytest에서 S3 UploadSourceDialog의 QTest.qWait 중 Windows access violation 기록 후 실행 미완료. 해당 테스트 단독 실행 1 passed, EC2/로그 관련 90 passed. | 전체 테스트 완료를 증명하지 못함; Qt event/widget 수명 및 테스트 간 간섭 확인 필요 | Phase 9 후속 검증; `.test-ec2-redesign-full.log` |
| QA-004 | Open | RDS static scan | 기존 ec2_rds.py 안내 widget 초기화 assert 2건이 Bandit B101에 걸림. Secrets 신규 범위 Bandit은 통과. | 전체 Bandit 게이트 정리 필요 | Phase 9 RDS 후속 검증 |
